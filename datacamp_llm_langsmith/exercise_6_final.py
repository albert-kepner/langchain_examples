...
def topic_similarity_evaluator(
    ...
    def get_topics(opinions):
        return [x["topic"] for x in opinions]
    topics = set(get_topics(outputs["opinions"] ))
    ref_topics = set(get_topics(reference_outputs["structured_review"]["opinions"] ))
    topic_intersect = topics & ref_topics
    topic_union = topics | ref_topics
    jaccard_sim = len(topic_intersect)/len(topic_union)
    return jaccard_sim
...
experiment_name = "app_review_opinions_evaluation"
